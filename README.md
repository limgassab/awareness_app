...
### Optional short pause after a difficult moment

The app does not show a separate pause feature on the home screen. After a difficult moment has been recorded, it gently asks whether a short pause would help. The user can start it or choose “Later”. Available actions include breathing slowly, taking a short walk, pausing in silence, writing, reflecting, talking to someone, or choosing a personal action.

After the pause, the person chooses only **Better**, **About the same**, or **More difficult**, with an optional note. This feature is optional and is not the central identity of the app.
...
- **Today / Aujourd’hui**: greeting, current intention, one clear **Record a moment** action, the fact of the day, and evening reflection.
- **Record a moment / Noter un moment**: the single entry feature for text, photo, or audio.
- **Journey / Parcours**: the chronological record of moments together with gentle summaries of intentions, reflections, and recurring themes.
...
```js
const CACHE_NAME = 'daily-reflection-cache-v4';
```

For a major published update, change the cache name to a new value, for example:

```js
const CACHE_NAME = 'daily-reflection-cache-v5';
```
